...
const features = [
  {
    ...
    description: {
      vi: "Hướng dẫn chi tiết cách xem báo cáo tổng quan và truy cập nhanh các chức năng.",
      ja: "概要レポートの表示方法と機能へのクイックアクセスの詳細ガイド。",
    },
    tourKey: "dashboard",
  ...
  },
  {
    ...
    actions: [
      ...
      },
      {
        ...
        internalPage: "content-create-post",
...
const GuideCenter = () => {
  ...
  return (
    <div className="p-4 sm:p-8 container mx-auto space-y-8 min-h-screen bg-gray-50/50">
      {/* Hero Header */}
      <div className="relative overflow-hidden rounded-3xl bg-linear-to-br from-indigo-600 via-blue-600 to-blue-500 shadow-xl">
        ...
        <div className="relative z-10 p-8 sm:p-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="text-white space-y-3 max-w-2xl">
            ...
            <p className="text-blue-100 text-lg leading-relaxed">
              {locale === "vi" ? (
                <>
                  Chào mừng bạn đến với Trung Tâm Hướng Dẫn hệ thống quản trị
                  website do Đội ngũ{" "}
                  <span className="font-bold text-white">W Four Tech</span> phát
                  triển. Hãy chọn một mục bên dưới để bắt đầu tour hướng dẫn
                  tương tác.
                </>
              ) : (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-2">
        {features.map((feature, index) => (
          <div
            key={index}
            className={`group relative bg-white rounded-2xl p-6 sm:p-8 shadow-sm hover:shadow-xl border border-gray-100 transition-all duration-300 hover:-translate-y-1 flex flex-col h-full overflow-hidden ${
              feature.actions ? "row-span-1 xl:col-span-1" : ""
            }`}
          >
            {/* Decoration gradient on hover */}
            <div
              className={`absolute top-0 left-0 w-full h-1 ${
                feature.btnColor?.replace("bg-", "bg-") || "bg-gray-200"
              } origin-left transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300`}
            ></div>

            <div className="mb-6 flex items-center justify-center">
              <div
                className={`${feature.bg} p-4 rounded-2xl transition-transform group-hover:scale-110 duration-300`}
              >
                {feature.icon}
              </div>
            </div>
            ...
            <p className="text-gray-500 text-sm leading-relaxed grow text-justify">
            ...
            <div className="mt-auto pt-4">
              {feature.actions ? (
                ...
                <button
                  ...
                  className={`w-full py-3 px-4 rounded-xl text-white font-medium shadow-sm hover:shadow-md transition-all flex items-center justify-center gap-2 group/btn ${
                    feature.btnColor || "bg-gray-600"
                  }`}
                >
