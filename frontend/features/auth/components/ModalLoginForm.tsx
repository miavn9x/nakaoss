...
export const ModalLoginForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isEmailFormat) {
      if (emailRef.current) {
        emailRef.current.setCustomValidity(t("invalidEmail"));
    ...
    if (!captchaInput.trim()) {
      if (captchaRef.current) {
        captchaRef.current.setCustomValidity(t("requiredCaptcha"));
    ...
    try {
      ...
      // 2024 Update: User info is now returned in login response for immediate UI update
      const userFromResponse = (res as any).data?.user;

      // Update Auth Context and Get User
      const user = await loginAuth(userFromResponse);
      ...
      if (
        ...
        (user.roles.includes("admin") ||
          user.roles.includes("cskh") ||
          user.roles.includes("employment"))
      ) {
      ...
      const errorMessage = error?.response?.data?.message || t("invalidLogin");
    ...
    <>
      ...
      <form className="flex flex-col gap-6 mt-2" onSubmit={handleSubmit}>
        ...
        <div className="group">
          <label
            htmlFor="email"
            className="block text-sm sm:text-[15px] font-bold text-gray-400 mb-1 group-focus-within:text-gray-600 transition-colors"
          >
            {t("emailLabel")}
          </label>
        ...
        >
          {isLocked
            ? t("locked", {
                seconds: Math.ceil((localLockUntil! - now) / 1000),
              })
            : isLoading
              ? t("processing")
              : t("login")}
