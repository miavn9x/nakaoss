  ...
  const validateField = (name: string, value: string) => {
    ...
    switch (name) {
      ...
      case "lastName":
        ...
        if (/\d/.test(value)) {
          error = t("nameNoNumbers");
        } else if (!/^[a-zA-Z\u00C0-\u024F\u1E00-\u1EFF\s]*$/.test(value)) {
          error = t("nameLettersOnly");
        }
        break;
      case "phoneNumber":
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!passwordRegex.test(formData.password)) {
      if (passwordRef.current) {
        passwordRef.current.setCustomValidity(t("passwordRequirement"));
    ...
    try {
      ...
      if (success === true) {
        ...
        // Update Global Auth State (Fetch Profile)
        // Since backend auto-logs in, we just need to fetch the user
        await loginAuth();
      ...
      <form className="flex flex-col gap-2 " onSubmit={handleSubmit}>
        <div className="flex gap-4">
          <div className="group flex-1">
            <label
              htmlFor="lastName"
              className="block text-sm sm:text-[15px] font-bold text-gray-400 group-focus-within:text-gray-600 transition-colors"
            >
              {t("lastName")}
            </label>
            <input
