    ...
    <>
      ...
      {leftAd && (
        <div
          ...
          ref={(el) => {
            if (el) {
              el.style.left = `calc(${leftAd.offsetPercent || 45}% - 948px)`;
              el.style.top = `${leftAd.offsetTop || 40}%`;
          ...
          {leftAd.link ? (
            <Link href={leftAd.link} target="_blank">
              <div
                ...
                ref={(el) => {
                  if (el) {
                    el.style.width = `${leftAd.width || 200}px`;
                    el.style.height = `${leftAd.height || 700}px`;
                  }
                }}
              >
                <Image
      ...
      {rightAd && (
        <div
          ...
          ref={(el) => {
            if (el) {
              el.style.right = `calc(${rightAd.offsetPercent || 45}% - 948px)`;
              el.style.top = `${rightAd.offsetTop || 40}%`;
              el.style.transform = "translateY(-50%)";
              el.style.zIndex = "40";
            }
          }}
          ...
          {rightAd.link ? (
            <Link href={rightAd.link} target="_blank">
              <div
                ...
                ref={(el) => {
                  if (el) {
                    el.style.width = `${rightAd.width || 200}px`;
            ...
            <div
              ...
              ref={(el) => {
                if (el) {
                  el.style.width = `${rightAd.width || 200}px`;
                  el.style.height = `${rightAd.height || 700}px`;
                }
              }}
            >
              <Image
      ...
      {mounted && popupAd && showPopup && (
        <div
          ...
          ref={(el) => {
            if (el) {
              el.style.paddingTop = `${(popupAd.offsetTop || 50) * 0.8}vh`;
            }
          }}
        >
          <div
            ...
            ref={(el) => {
              if (el) {
                el.style.width = `${popupAd.width || 500}px`;
