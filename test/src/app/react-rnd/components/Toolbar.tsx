    ...
    <div className="editor-toolbar bg-white/80 backdrop-blur-xl border-b border-slate-200/80 px-8 py-4 flex items-center justify-between shadow-[0_4px_20px_rgb(0,0,0,0.03)] sticky top-0 z-50">
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-1">
          <button
            onClick={onUndo}
            disabled={!canUndo}
            className={`p-2 rounded-xl transition-all ${canUndo
          ...
          <button
            onClick={onRedo}
            disabled={!canRedo}
            className={`p-2 rounded-xl transition-all ${canRedo
              ? "text-slate-600 hover:bg-slate-100 hover:text-indigo-600"
              : "text-slate-300 cursor-not-allowed"
              }`}
            title="Redo (Ctrl+Y)"
        ...
        <button
          ...
          className={`p-2 rounded-xl transition-all flex items-center gap-1.5 shadow-sm border ${showGrid
        ...
        {showGrid && (
          <div className="flex items-center gap-1.5 ml-1 animate-in fade-in slide-in-from-left-2 duration-200">
            <input
              ...
              className="w-9 h-9 rounded-lg shrink-0 cursor-pointer border border-slate-200 p-0.5 shadow-sm bg-white"
              title="Đổi màu lưới"
            />
          </div>
        )}
      ...
      <div className="flex items-center justify-end gap-2 sm:gap-4 ml-auto">
        ...
        <button
          ...
          className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-xl transition-all text-sm shadow-md shadow-indigo-200 flex items-center gap-2 border border-indigo-600"
          title="Thêm một Nút bấm (Button)"
        >
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2" /><path d="M12 8v8" /><path d="M8 12h8" /></svg>
          Thêm Nút
        </button>
